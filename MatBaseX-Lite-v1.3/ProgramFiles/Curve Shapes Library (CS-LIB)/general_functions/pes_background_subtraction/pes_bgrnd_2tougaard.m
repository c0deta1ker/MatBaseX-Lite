function [roi_xdat, roi_ydat, roi_bgrnd] = pes_bgrnd_2tougaard(xdat, ydat, LHS, RHS, Win, C, D)
% [roi_xdat, roi_ydat, roi_bgrnd] = pes_bgrnd_2tougaard(xdat, ydat, LHS, RHS, Win, C, D)
...
[~, ~, Y_Bgrnd{1}]    = pes_bgrnd_linear(xdat, ydat, LHS, RHS, Win);
...
while cnvgnce > 1e-12
    ...
    N = N + 1;
    % --- Finding the data parity type (positive or negative x-axis)
    if mean(xdat) > 0;  data_parity = 1;
    else;               data_parity = -1;
    end
    if data_parity == 1
        % --- Determine total integrated area
        dE      = (roi_xdat - roi_xdat(end));
        Fn      = (roi_ydat) .*dE ./ ((C + dE.^2).^2 + (D.*dE.^2));
        Kn      = (I1 - I2) ./ trapz(roi_xdat, Fn);
        % --- Determine running integral (integrated area from left to right)
        Q       = zeros(size(Y_Final{1}));
        for i = 1:nRows-1
            dEi     = (roi_xdat(end-i:end) - roi_xdat(i));
            Fi      = (roi_ydat(end-i:end)) .* dEi ./ ((C + dEi.^2).^2 + (D.*dEi.^2));
            Q(i+1)	= trapz(roi_xdat(end-i:end), Fi);
        end
        Q(1)	= 0;

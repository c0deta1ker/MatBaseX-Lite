...
if plot_results == 1
    ...
    % - Plot versus theta
        if isscalar(phi)
            ...
            axis([0, 90, 0, 3.75]);
            text(0.98, 0.91, sprintf("P = %.2f, hv = %.0f eV, phi = %.0f deg.", P, hv(1), phi(1)),...
    ...
    % - Plot versus phi
            ...
            ylabel(' Angular Anisotropy (FP) ', 'FontWeight','bold');
            axis([0, 180, 0, 3.75]);
            text(0.98, 0.91, sprintf("P = %.2f, hv = %.0f eV, theta = %.0f deg.", P, hv(1), theta(1)),...
                'FontSize', 8, 'color', 'k', 'Units','normalized', 'HorizontalAlignment','right');
        end
    else
    % - Otherwise, plot versus photon energy
        ...
        axis([850, 10150, 0, 3.75]);
            text(0.98, 0.91, sprintf("P = %.2f, theta = %.0f deg., phi = %.0f deg.", P, theta(1), phi(1)),...
                'FontSize', 8, 'color', 'k', 'Units','normalized', 'HorizontalAlignment','right');
    end
    % - Formatting the axis
    ...
    text(0.98, 0.94, sprintf("Cant(2022) - Partially Polarized Light (FP)"),...
        'FontSize', 8, 'color', 'k', 'Units','normalized', 'HorizontalAlignment','right');
